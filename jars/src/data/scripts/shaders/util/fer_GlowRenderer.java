...
public class fer_GlowRenderer extends BaseRenderPlugin {
    ...
    protected void updateBuffers(int[] buffers, CombatEngineLayers combatEngineLayers, ViewportAPI viewport) {
        ...
        for (fer_EnginePlugin.FlareData flare : drawTargets) {
            Matrix4f modelView = flare.flare.getModelViewGlow(view);
    ...
    protected ShaderProgram initShaderProgram() {
        ...
        try {
            vert = Global.getSettings().loadText("data/shaders/glow.vert");
            frag = Global.getSettings().loadText("data/shaders/glow_optimised.frag");
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
